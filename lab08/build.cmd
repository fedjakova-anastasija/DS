...
echo VowelConsCounter:3 >> %newVersion%/config/config.txt 
echo VowelConsRater:2 >> %newVersion%/config/config.txt 
echo TextLimit:1 >> %newVersion%/config/config.txt 
...
dotnet publish -c Release -o ../../%newVersion%/TextStatistics
if %ERRORLEVEL% NEQ 0 (
    echo Unsuccessful build
	exit /b 0
)
...
dotnet publish -c Release -o ../../%newVersion%/TextProcessingLimiter
if %ERRORLEVEL% NEQ 0 (
    echo Unsuccessful build
	exit /b 0
)
